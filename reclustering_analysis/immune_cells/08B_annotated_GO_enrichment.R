...
dir.create("08_GO_annotated")
setwd("08_GO_annotated")


# LOAD DEGS  ----------------------------------------------------

temp_cluster.allmarkers <- 
  read.csv(paste0("../07_annotated_objects/03_DGE/01_MAST_FINDALLMARKERS_RNA_annotated_celltype_unassignedremoved.csv"))

# filter for p_val adjusted
temp_cluster.allmarkers <- temp_cluster.allmarkers[temp_cluster.allmarkers$p_val_adj < 0.05,,drop=F]
